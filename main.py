...
from models.dataset import ImageHTMLDataSet, Collator, make_datasets
...
def get_models(args):
    ...
    decoder.out = nn.Linear(args.dim_reformer, 2)
...
def get_schedulers(args, opt_dec, step_max):
    ...
    return scheduler_dec


#def train(batch_size, encoder, decoder, resnet, args):
def train(batch_size, decoder, args):
    ...
    scheduler_dec = get_schedulers(args, opt_dec, step_scheduler)
    ...
    while (step_global < args.step_max):
        for (feature, y_in, y_fake, y_label, lengths,
            ...
            y_fake = y_fake.to(args.device, non_blocking=True)
            y_label = y_label.to(args.device, non_blocking=True)
            ...
            y_out = decoder(y_fake, return_loss=False)
            loss = F.cross_entropy(y_out.transpose(1, 2), y_label)
            ...
            if (step_global + 1) % args.step_valid == 0:
                ...
                loss_valid = validate(args.dataloader_valid, decoder, args,
                                      step_global, ce_weight)
                if loss_valid < loss_min:
                    loss_min = loss_valid
                    step_best = step_global + 1
                    # save_models(args, step_global + 1, encoder, decoder,
                    #             resnet)
                    save_models(args, step_global + 1, decoder)
...
def save_models(args, step, decoder):
...
def validate(dataloader, decoder, args, step, ce_weight=None):
    #encoder.eval()
    decoder.eval()
    #resnet.eval()
    eval_losses = AverageMeter()

    for i, (feature, y_in, y_fake, y_label, lengths,
...
if __name__ == '__main__':
    ...
    # dataset
    args.my_collator = Collator(args.seq_len, vocab_size=304)
